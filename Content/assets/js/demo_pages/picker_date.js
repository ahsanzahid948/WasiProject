...
var DateTimePickers = function() {
    ...
    var _componentDaterange = function() {
        ...
        $('.daterange-single').daterangepicker({
            singleDatePicker: true, showDropdowns: true/*, autoUpdateInput: false*/, autoClose: true, locale: { format: 'DD-MM-YYYY' }
        }//, //function (chosen_date) {                    $(this.element[0]).val(chosen_date.format('DD-MM-YYYY')); }
                );
    ...
    /*NOTE:*** 11-22-2019 for date range picker format (DD-MM-YYYY) by class implementation*/
    /*NOTE:*** Ahmed Raza*/
    var rangeDateDDMMYYYY = '%d-%m-%Y';
    var rangeConvDDMMYYYY = new AnyTime.Converter({ format: rangeDateDDMMYYYY });
    // Set today's date
    $('.btnRToday').on('click', function (e) { $('.clsRSD').val(rangeConvDDMMYYYY.format(new Date())).trigger('change'); });
    // Clear dates
    $('#btnRClear').on('click', function (e) { $('.clsRSD').val('').trigger('change'); });
    // Start date
    $('.clsRSD').AnyTime_picker({ format: rangeDateDDMMYYYY });
    $('.clsRSD').on('change', function (e) {
        try {
            debugger;
            var fromDay = rangeConvDDMMYYYY.parse($('.clsRSD').val()).getTime();
            var dayLater = new Date(fromDay);// +oneDay
            dayLater.setHours(0, 0, 0, 0);
            var ninetyDaysLater = new Date(); //fromDay + (90 * oneDay)
            ninetyDaysLater.setHours(23, 59, 59, 999);
            ...
            let rsdval = $(this).val(),
                redval = $('.clsRED').val();
            if (rsdval > redval || redval == null || redval == '' || redval == undefined) {
                $('.clsRED').AnyTime_noPicker().removeAttr('disabled')
                .val(rangeConvDDMMYYYY.format(dayLater))
                .AnyTime_picker({
                    earliest: dayLater, format: rangeDateDDMMYYYY//,
                    //latest: ninetyDaysLater
                });
            }
        }
        catch (e) { // Disable End date field
            $('.clsRED').val('').attr('disabled', 'disabled');
        }
    });
    /**/
    /*NOTE:*** 02-12-2019 for single date picker format (DD-MM-YYYY) by class implementation*/
    /*NOTE:*** Ahmed Raza*/
    // Set today's date
    $('.btnSingleDateToday').on('click', function (e) { $('.clsSingleDate').val(rangeConvDDMMYYYY.format(new Date())).trigger('change'); });
    // Clear dates
    $('#btnSingleDateClear').on('click', function (e) { $('.clsSingleDate').val('').trigger('change'); });
    // Start date
    $('.clsSingleDate').AnyTime_picker({ format: rangeDateDDMMYYYY });
    $('.clsSingleDate').on('change', function (e) {
        try {
            var fromDay = rangeConvDDMMYYYY.parse($('.clsSingleDate').val()).getTime();
            var dayLater = new Date(fromDay);// +oneDay
            dayLater.setHours(0, 0, 0, 0);
            var ninetyDaysLater = new Date(); //fromDay + (90 * oneDay)
            ninetyDaysLater.setHours(23, 59, 59, 999);
